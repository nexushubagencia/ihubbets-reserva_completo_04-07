...
class InsertMatchBasketBall extends Command
{
    protected $signature = 'command:insert-match-basketball';
    protected $description = 'Importa partidas de basquete da BetsAPI para o IHUB';
    ...
    public function handle()
    {
        try {
            ...
            if (!$settings || $settings->op_basquete !== 'Sim') {
                $this->info("Basquete desativado para este site.");
                return;
            }

            $this->info("Iniciando importação de partidas de Basquete (site_id: {$siteId})...");
            ...
            $response = $this->api->getUpcomingEvents(18, 1);
            ...
            for ($i = 1; $i <= $totalPages; $i++) {
                ...
                $pageResponse = $this->api->getUpcomingEvents(18, $i);
                ...
                foreach ($pageResponse['results'] as $match) {
                    ...
                    if ($existingMatch) {
                        ...
                        if ($matchCad) {
                            $this->fetchAndSetResultData($matchCad, $match->id, $siteId);
            ...
            $this->info("Importação de Basquete concluída com sucesso!");

        } catch (\Exception $e) {
            Log::error("Erro no InsertMatchBasketBall: " . $e->getMessage());
            $this->error("Erro: " . $e->getMessage());
        }
    }

    private function fetchAndSetResultData(MatchModel $matchCad, int $eventId, int $siteId): void
    {
        try {
