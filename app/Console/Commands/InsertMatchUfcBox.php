...
class InsertMatchUfcBox extends Command
{
    protected $signature = 'command:insert-match-ufcbox';
    protected $description = 'Importa partidas de UFC/Boxe da BetsAPI para o IHUB';
    ...
    public function handle()
    {
        try {
            ...
            if (!$settings || $settings->op_ufcbox !== 'Sim') {
                $this->info("UFC/Boxe desativado para este site.");
                return;
            }

            $this->info("Iniciando importação de partidas de UFC/Boxe (site_id: {$siteId})...");
            ...
            $response = $this->api->getUpcomingEvents(9, 1);
            ...
            for ($i = 1; $i <= $totalPages; $i++) {
                ...
                $pageResponse = $this->api->getUpcomingEvents(9, $i);
                ...
                foreach ($pageResponse['results'] as $match) {
                    ...
                    if ($existingMatch) {
                        ...
                        $matchCad = MatchModel::create([
                            ...
                            'home'          => $match->home->name ?? 'Lutador A',
                            'image_id_home' => null,
                            'away'          => $match->away->name ?? 'Lutador B',
                            'image_id_away' => null,
                            'live_status'   => null,
                        ]);
            ...
            $this->info("Importação de UFC/Boxe concluída com sucesso!");

        } catch (\Exception $e) {
            Log::error("Erro no InsertMatchUfcBox: " . $e->getMessage());
