    ...
    foreach ($sportsId as $key => $sport) {
      ...
      for ($pageUpCommings=1; $pageUpCommings <= $upCommingsPages; $pageUpCommings++) {
        ...
        foreach ($upCommings->results as $upComming) {
          ...
          $lastMatachCreated = MatchModel::updateOrCreate([
              ...
              if (isset($market->market->marketType) && $market->market->marketType == "MATCH_ODDS") {
                foreach ($market->market->runners as $key => $runners) {
                  if (isset($market->runnerDetails[$key]->runnerOdds->decimalDisplayOdds->decimalOdds)) {
                    Odd::create([
                      ...
                      'value'                   => $market->runnerDetails[$key]->runnerOdds->decimalDisplayOdds->decimalOdds,
                      'status'                    => 1,
                ...
                for ($i = 0; $i < count($market->market->runners); $i++) {
                  Odd::create([
                    ...
                    'event_id'                  =>  $upComming->id,
                    'market_name'              =>  'Ambas as equipes marcarão na partida',
                ...
                for ($i = 0; $i < count($market->runnerDetails); $i++) {
                  $odd = Odd::where('event_id', $upComming->id)->where('market_name', 'Ambas as equipes marcarão na partida')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                  if ($odd) {
                    $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                    $odd->status = 1;
                    $odd->save();
                  }
                }
                ...
                for ($i = 0; $i < count($market->market->runners); $i++) {
                  ...
                  for ($i = 0; $i < count($market->runnerDetails); $i++) {
                    $odd = Odd::where('event_id', $upComming->id)->where('market_name', 'Chance Dupla')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                    if ($odd) {
                      $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                      $odd->status = 1;
                      $odd->save();
                ...
                for ($i = 0; $i < count($market->market->runners); $i++) {
                  $odd = Odd::create([
                    'match_id'                  =>  $lastMatachCreated->id,
                    'event_id'                  =>  $upComming->id,
                    'market_name'              =>  'Placar Exato Tempo Completo',
                ...
                //Cotação
                for ($i = 0; $i < count($market->runnerDetails); $i++) {
                  $odd = Odd::where('event_id', $upComming->id)->where('market_name', 'Placar Exato Tempo Completo')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                  if ($odd) {
                    $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                    $odd->status = 1;
                    $odd->save();
                ...
                $delete = Odd::where('event_id', $upComming->id)->where('market_name', 'Placar Exato Tempo Completo')->where('status', 0)->delete();
              } elseif (isset($market->market->marketType) && $market->market->marketType == "HALF_TIME_SCORE") {
                for ($i = 0; $i < count($market->market->runners); $i++) {
                  $odd = Odd::create([
                    'match_id'                  =>  $lastMatachCreated->id,
                    'event_id'                  =>  $upComming->id,
                    'market_name'              =>  'Placar Exato (1T)',
                ...
                //Cotação
                for ($i = 0; $i < count($market->runnerDetails); $i++) {
                  $odd = Odd::where('event_id', $upComming->id)->where('market_name', 'Placar Exato (1T)')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                  if ($odd) {
                    $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                    $odd->status = 1;
                    $odd->save();
                ...
                $delete = Odd::where('event_id', $upComming->id)->where('market_name', 'Placar Exato (1T)')->where('status', 0)->delete();
              } elseif (isset($market->market->marketType) && $market->market->marketType == "2ND_HALF_CORRECT_SCORE") {
                for ($i = 0; $i < count($market->market->runners); $i++) {
                  $odd = Odd::create([
                    'match_id'                  =>  $lastMatachCreated->id,
                    'event_id'                  =>  $upComming->id,
                    'market_name'              =>  'Placar Exato (2T)',
                ...
                //Cotação
                for ($i = 0; $i < count($market->runnerDetails); $i++) {
                  $odd = Odd::where('event_id', $upComming->id)->where('market_name', 'Placar Exato (2T)')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                  if ($odd) {
                    $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                    $odd->status = 1;
                    $odd->save();
                ...
                $delete = Odd::where('event_id', $upComming->id)->where('market_name', 'Placar Exato (2T)')->where('status', 0)->delete();
              }
            }
          }
        }
