...
class LiveAfterTomorow extends Command
{
    ...
    public function showDepoisAmanha()
    {
        ...
        $mainleagues = MainLeague::where('site_id', config('tenant.site_id', 1))->get();
        ...
        //config gerais
        $league_bloqueadas = BlockLeague::where('site_id', config('tenant.site_id', 1))->get();
        ...
        $merc_blocks = ConfigMercados::where('status', 0)
            ...
            ->where('user_id', config('tenant.user_id', 1))
            ->get();
        ...
        $odd_zerada = Configuracao::where('site_id', config('tenant.site_id', 1))
            ->get();
        ...
        //Odds alteradas
        $odds_alteradas = BlockOddMatch::where('site_id', config('tenant.site_id', 1))
            ->where('status', 1)
            ->get();
        ...
        $odds_bloqueadas = BlockOddMatch::where('site_id', config('tenant.site_id', 1))
            ->where('status', 0)
            ->get();
                        ...
                        if (in_array('Vencedor do Encontro', $block_merc)) {
                            ...
                            if (in_array($odd->id, $arr_odd_b)) {
                                ...
                                if ($odd->value <= $odd_z) {
                                    ...
                                    $m_porcents_geral = ConfigMercados::where('site_id', config('tenant.site_id', 1))
                                        ->where('name', $odd->market_name)
                                        ->where('status', 1)
                                        ->where('user_id', config('tenant.user_id', 1))
                                        ->first();
                                    ...
                                    $o_porcents_geral = ConfigOdd::where('site_id', config('tenant.site_id', 1))
                                        ->where('status', 1)
    ...
    {
        $matchs =  AferTomorowMatchFlash::where('site_id', config('tenant.site_id', 1))->first();
