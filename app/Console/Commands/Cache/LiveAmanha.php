...
class LiveAmanha extends Command
{
    ...
    protected $description = 'Monta cache dos jogos ao vivo de amanhã para todos os esportes com comissão aplicada';
    ...
    public function handle()
    {
        ...
        try {
            $this->info("Iniciando LiveAmanha (site_id: {$siteId})...");
            ...
            $dataAmanha = Carbon::tomorrow()->format('Y-m-d');
            $inicio    = $dataAmanha . ' 00:00:00';
            $fim       = $dataAmanha . ' 23:59:59';
            ...
            $cacheKey = "live_amanha_{$siteId}";
            Cache::put($cacheKey, $return, now()->addMinutes(5));

            $totalMatches = collect($return)->sum(fn($l) => count($l['match'] ?? []));
            $this->info("LiveAmanha concluído: " . count($return) . " ligas, {$totalMatches} jogos. Cache: {$cacheKey}");

            return $return;

        } catch (\Exception $e) {
            Log::error("Erro no LiveAmanha: " . $e->getMessage());
