...
class InsertMatchEsports extends Command
{
    protected $signature = 'command:insert-match-esports';
    protected $description = 'Importa partidas de E-Sports da BetsAPI para o IHUB';
    ...
    public function handle()
    {
        try {
            ...
            if (!$settings || $settings->op_e_sports !== 'Sim') {
                $this->info("E-Sports desativado para este site.");
                return;
            }

            $this->info("Iniciando importação de partidas de E-Sports (site_id: {$siteId})...");
            ...
            $response = $this->api->getUpcomingEvents(151, 1);
            ...
            for ($i = 1; $i <= $totalPages; $i++) {
                ...
                $pageResponse = $this->api->getUpcomingEvents(151, $i);
            ...
            $this->info("Importação de E-Sports concluída com sucesso!");

        } catch (\Exception $e) {
            Log::error("Erro no InsertMatchEsports: " . $e->getMessage());
