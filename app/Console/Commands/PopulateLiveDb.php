                    ...
                    foreach ($markets as $market) {
                      if (isset($market->market->marketType) && $market->market->marketType == "MATCH_ODDS") {
                        ...
                        for ($i = 0; $i < count($market->runnerDetails); $i++) {
                          $odd = Odd::where('event_id', $result->id)->where('market_name', 'Ambas as equipes marcarão na partida')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                          if ($odd) {
                        ...
                        for ($i = 0; $i < count($market->market->runners); $i++) {
                          ...
                          for ($i = 0; $i < count($market->runnerDetails); $i++) {
                            $odd = Odd::where('event_id', $result->id)->where('market_name', 'Chance Dupla')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                            if ($odd) {
                              $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                              $odd->status = 1;
                              $odd->save();
                        ...
                        //Cotação
                        for ($i = 0; $i < count($market->runnerDetails); $i++) {
                          $odd = Odd::where('event_id', $result->id)->where('market_name', 'Placar Exato Tempo Completo')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                          if ($odd) {
                            $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                            $odd->status = 1;
                            $odd->save();
                        ...
                        //Cotação
                        for ($i = 0; $i < count($market->runnerDetails); $i++) {
                          $odd = Odd::where('event_id', $result->id)->where('market_name', 'Placar Exato (1T)')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                          if ($odd) {
                            $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                            $odd->status = 1;
                            $odd->save();
                        ...
                        //Cotação
                        for ($i = 0; $i < count($market->runnerDetails); $i++) {
                          $odd = Odd::where('event_id', $result->id)->where('market_name', 'Placar Exato (2T)')->where('selectionId', $market->runnerDetails[$i]->selectionId)->first();
                          if ($odd) {
                            $odd->value = $market->runnerDetails[$i]->runnerOdds->decimalDisplayOdds->decimalOdds;
                            $odd->status = 1;
                            $odd->save();
