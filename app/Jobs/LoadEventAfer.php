...
class LoadEventAfer implements ShouldQueue
{
    ...
    public function showAfer()
    {
        $siteId  = $this->siteId;
        $afer    = Carbon::tomorrow()->addDay();
        ...
        $dataAfer = $afer->format('Y-m-d');

        $leagues = MatchModel::select('league')
            ->where('site_id', $siteId)
            ->where('date', '>=', $dataAfer . ' 00:00:00')
            ->where('date', '<=', $dataAfer . ' 23:59:59')
            ->where('visible', 'Sim')
            ...
            foreach ($matchs as $match) {
                ...
                foreach ($odds as $odd) {
                    ...
                    $return[$i]['match'][$j]['odds'][$q] = [
                        'id' => $odd->id, 'group_opp' => $odd->market_name, 'odd' => $odd->label,
                        'value' => $odd->value, 'cotacao' => $cotacao, 'type' => $odd->type,
                    ];
                    $q++;
                }
                ...
                if (empty($return[$i]['match'][$j]['odds'])) {
                    for ($q = 0; $q < 3; $q++) {
                        $return[$i]['match'][$j]['odds'][$q] = [
                            ...
                            'group_opp' => 'Vencedor do Encontro', 'odd' => $q,
                            'value' => 0, 'cotacao' => 0, 'type' => 'pre',
                        ];
                    }
                }
    ...
    {
        $data = $this->showAfer();
        Cache::put("home_afer_{$this->siteId}", $data, now()->addMinutes(5));
        return $data;
    }
}
