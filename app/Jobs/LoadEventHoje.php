...
class LoadEventHoje implements ShouldQueue
{
    ...
    public function showHoje()
    {
        ...
        $dataHoje = $this->hoje->format('Y-m-d');

        $leagues = MatchModel::select('league')
            ->where('site_id', $siteId)
            ->where('date', '>=', $this->agora)
            ->where('date', '<', $dataHoje . ' 23:59:00')
            ->where('visible', 'Sim')
        ...
        foreach ($leagues as $league) {
            ...
            foreach ($matchs as $match) {
                ...
                foreach ($odds as $odd) {
                    ...
                    if (in_array($key, $blockedOdds)) {
                        $cotacao = 0;
                    } elseif (isset($alteredOdds[$key])) {
                        $cotacao = round($alteredOdds[$key], 2);
                    } else {
                        if ($cotacao <= $oddZ) {
                            $cotacao = 0;
                        } elseif ($cotacao > $oddM) {
    ...
    {
        return $this->showHoje();
    }

    public function handle()
    {
        $data = $this->showHomeHoje();
        Cache::put("home_hoje_{$this->siteId}", $data, now()->addMinutes(5));
        return $data;
    }
}
